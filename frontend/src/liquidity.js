...
async function handleChange(event) {
    ...
    const values = await spaceRouterContract.quote(amountInA, amountInB, overrides);
    $("#base-token").val(formatEther(values.amountA));
    $("#target-token").val(formatEther(values.amountB));
}
...
async function handleConnect(event) {
  ...
  const transactionResponse = await spaceRouterContract.connect(signer).addLiquidity(amountInB, overrides);
